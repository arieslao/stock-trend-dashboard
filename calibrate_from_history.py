...
def main():
    ...
    if any(c not in header for c in need):
        print("Predictions sheet missing some columns; skipping calibration."); return
    i = {c: header.index(c) for c in header}
    rows = vals[1:]

    df = pd.DataFrame(
        [[r[i["timestamp_utc"]], r[i["symbol"]], r[i["predicted"]], r[i["actual"]],
          r[i["error_pct"]], r[i["evaluated_at_utc"]]] for r in rows if len(r) >= len(header)],
    ...
    if df.empty:
        print("No evaluated predictions in lookback window."); return
    ...
    if grp.empty:
        print("Not enough samples per symbol for calibration."); return
    ...
    rows = [[r["symbol"], round(float(r["bias_pct"]), 4), int(r["n_samples"]), now_iso, args.lookback_days]
            for _, r in grp.iterrows()]

    # replace whole sheet contents (header + rows) to avoid stale symbols
    wc.clear()
